...
const updatepass = async (req, res, next) => {
  ...
  try {
    ...
    // Compare the entered password with the agent's current hashed password
    ...
    console.error('Error occurred while updating agent password:', error);
    res.status(500).json({
      message: 'Error occurred while updating agent password!',
    });
  }
};
...
const resetPassword = async (req, res) =>{
  ...
  try {
    ...
    agent.password = mdpEncrypted;
    await agent.save();

    // Send the new password to the chauffeur via email or other means
    sendpassword(email , newPassword , agentName);
...
async function sendpassword(Email ,Password , agentName) {
  // create reusable transporter object using the default SMTP transport
  let transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
...
const destroy = async (req, res) => {
  ...
  module.exports ={
    register, login,recupereruse,updateuse,destroy,searchuse,update,updatestatus,agentdes,updatestatuss,updatepass,resetPassword
    }